...
| `docs/deployment.md` | production deploy: the deployment URL per host (§2), sub-path hosting, Cloudflare Workers Builds + `wrangler.jsonc` (§4.3), GitHub Actions, Node pin, DoD gate |
...
- **Build & deployment (Plan D)** — the artifact is **portable**: one `dist/` runs at any origin and any sub-path, so nothing may assume it is served from `/`. `npm run build` is `astro check && astro build && node scripts/portablize.mjs`, and that third command rewrites every navigational URL in `dist/` to a document-relative one — which is why **`base` is never set** (Astro cannot emit relative URLs). It fails the build, not the review, if a root-absolute URL survives in a page or stylesheet, if a chunk carries a root-absolute literal or assembles a root base at runtime (`vite.experimental.renderBuiltUrl` in `astro.config.mjs` is the pin for the second), or if a page ships without its one `[data-site-root]` anchor. **A link built in client JS must resolve against that anchor** — `siteRoot()` + `lessonHref()`/`reviewHref()` in `src/lib/progress.ts` — never as a `/learn/${slug}/` literal. `dist/404.html` is the one document that keeps root-absolute links (it is served at the URL the reader typed) and gets the deployment's base path prefixed instead. **Declared** URLs (canonical, `og:url`, `og:image`, sitemap `<loc>`, robots' `Sitemap:`, JSON-LD `url`) are the opposite case: absolute, built only by `deploymentUrl()` in `src/lib/deployment-url.ts` — `new URL(path, site)` discards a sub-path — from `SITE_URL` or the sentinel `https://learndsa.invalid`; a build on any **publishing** platform without `SITE_URL` throws — `astro.config.mjs`'s `PUBLISHING_BUILDERS` list covers `CF_PAGES`, `WORKERS_CI` (this site deploys through Cloudflare **Workers Builds**, not Pages), `NETLIFY` and `VERCEL`, and `tests/unit/rehost.test.ts` pins all four — and `npm run rehost <url>` stamps a built `dist/` for hosts with no build step (it refuses to run twice). **`wrangler.jsonc` is committed and must stay so:** without a Wrangler config `wrangler deploy` auto-configures the project, installing the spec-§4-forbidden `@astrojs/cloudflare` adapter and moving the output to `dist/client/`, and its two `assets` handling keys are what give Workers the trailing-slash redirect and the 404 page. `tests/e2e/portable.spec.ts` serves the same build under a two-segment sub-path on :4322. See `docs/deployment.md` §2 and amendment U-2; the three accepted limitations (sub-path `robots.txt`, origin-scoped `localStorage`, mirrors competing) are §2.4.
