 ...
 *
 * REPRODUCIBILITY, fixed by the 2026-08 redesign. This used to carry a caveat:
 * the card set `--font-sans`/`--font-mono`, which were system stacks, so the
 * wordmark and headline were drawn in whatever the GENERATING machine resolved
 * `system-ui` to and re-running on another OS re-lettered the card. The
 * alternative was "an embedded webfont — a new asset the spec does not ask for".
 *
 * The site now HAS that asset: `public/fonts/*.woff2`, generated by
 * `npm run fonts` and already committed. So the two faces are inlined into the
 * card's SVG as base64 `@font-face` rules below, which costs the shipped site
 * nothing (the output is a PNG) and buys two things at once — the card is
 * lettered in the brand's own type rather than the generator's, and it renders
 * byte-identically on any machine that runs this script.
 *
...
/**
 * The two self-hosted faces, inlined so the card is lettered in the brand's type
 * on every machine. Read from `public/fonts/`, which `npm run fonts` writes and
 * `tests/unit/font-charset.test.ts` guards — never from a CDN, and never
 * re-declared here: the family names must match the ones in `tokens.css`, since
 * `--font-sans` is read out of that file three lines below.
 */
const FONT_FACES = [
  { family: 'IBM Plex Sans', file: 'plex-sans.woff2', weights: '100 700' },
  { family: 'IBM Plex Mono', file: 'plex-mono.woff2', weights: '400 600' },
]
  .map(({ family, file, weights }) => {
