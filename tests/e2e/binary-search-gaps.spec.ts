...
async function runCustom(page: Page, array: string, target: string) {
  await page.locator('[data-viz-array]').fill(array);
  await page.locator('[data-viz-target]').fill(target);
...
test.describe('Binary Search — absent target (empty-range "not found")', () => {
  test('ends with a "not in the array" explanation and NO found cue anywhere', async ({
    ...
    await expect(page.locator('[data-viz] .is-found')).toHaveCount(0);
    await expect(page.locator('[data-viz] .viz-found-mark')).toHaveCount(0);

    // The terminal window is empty, so every cell reads as eliminated (dimmed).
    await expect(page.locator('[data-viz] .viz-cell')).toHaveCount(5);
    await expect(
      page.locator('[data-viz] .viz-cell.is-eliminated'),
    ).toHaveCount(5);
  });
});
...
test.describe('Binary Search — scrub to an arbitrary step', () => {
  test('slider jump syncs explanation, aria-valuetext, counter, and SVG state', async ({
    ...
    await expect(page.locator('[data-viz-counter]')).toHaveText('2 / 4');
    ...
    await expect(page.locator('#i2')).toHaveClass(/is-active/);
    await expect(page.locator('[data-viz] .viz-mid-label')).toHaveText('mid');
  });
});
...
test.describe('Binary Search — step-back after play (deep-copy integrity)', () => {
  test('stepping back from the found end restores the earlier window exactly', async ({
    ...
    await expect(page.locator('#i3')).toHaveClass(/is-found/);
    ...
    await expect(page.locator('[data-viz-counter]')).toHaveText('3 / 4');
    await expect(page.locator('[data-viz-explain]')).toContainText(
      'index 4 holds 9',
    );
    // The found highlight must be gone and the historical mid restored on i4.
    await expect(page.locator('[data-viz] .is-found')).toHaveCount(0);
    await expect(page.locator('#i4')).toHaveClass(/is-active/);
  });
});
...
test.describe('Binary Search — non-color cues (design §3.4)', () => {
  test('range/active/found each pair color with a label, dim, or ✓ glyph', async ({
    ...
    await expect(page.locator('#i0')).toHaveClass(/is-eliminated/);
    ...
    await page.locator('[data-viz-slider]').fill('3');
    await expect(page.locator('[data-viz] .viz-found-mark')).toHaveText('✓');
    await expect(page.locator('#i3')).toHaveClass(/is-found/);
  });
});
...
test.describe('Binary Search — keyboard-only operation', () => {
  ...
  test('transport buttons activate with the keyboard (focus + Enter)', async ({
    ...
    const reset = page.locator('[data-viz-reset]');
    await reset.focus();
    await page.keyboard.press('Enter');
    await expect(page.locator('[data-viz-counter]')).toHaveText('1 / 4');
...
test.describe('Binary Search — metrics readout', () => {
  test('the comparisons metric increases as the trace advances', async ({
    ...
    const metrics = page.locator('[data-viz-metrics]');
